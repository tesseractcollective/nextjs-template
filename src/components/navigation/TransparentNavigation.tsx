...
export default function TransparentNavigation({
  navigations,
  siteLibrary,
  hideNav,
  ...
  useEffect(() => {
    ...
    const updateSmall = () => {
      ...
      // Set buffer zones: 300px to activate, 200px to deactivate
  ...
  return (
    <>
      ...
      <header
        className={`default-navigation fixed ${
          navigation?.announcementText ? "top-8" : "top-0"
        } z-[999] left-0 right-0  transition-all duration-[400ms] border-b  ${
          small
            ? "nav-shadow-scrolled backdrop-blur-md bg-after-nav border-primary nav-shadow black-shadow"
            : "border-[#00000000]"
      ...
      >
        ...
        <nav aria-label="Top" className="relative z-2 mx-auto max-w-10xl px-4">
          <div
            className={`w-full justify-between grid grid-cols-2 xl:grid-cols-3 transition-all duration-[400ms] ${
              small ? "py-4" : "py-6"
            }`}
          >
            {/* LOGO */}
              ...
              {navigation?.navigationLogo ? (
                <>
                  ...
                  <Fade direction="down" triggerOnce>
                    <Image
                      className={`w-auto object-contain max-w-[240px] xl:max-w-[288px] transition-all ${
                        small
                          ? "h-8 xl:h-10" // 2rem = h-8, 2.5rem = h-10
                          : "h-16 xl:h-20" // 4rem = h-16, 5rem = h-20
                      }`}
            ...
            {/* Middle nav items */}
            <div className="hidden h-full xl:flex justify-center">
                            ...
                            <Popover className="flex">
                              {({ open }) => (
                                <>
                                  ...
                                  >
                                    <Popover.Panel className="absolute inset-x-0 top-[100%] text-xs lg:text-sm xl:text-base text-text-color box-shadow max-w-4xl mx-auto rounded-xl z-30">
                                      {({ close }) => (
                                        <>
            ...
            <div className="flex flex-1 items-center justify-end max-w-max ml-auto">
                ...
                <div className="hidden xl:flex items-center justify-end">
                  {primaryItems.map((mainNavigationItem) => (
                    <LinkItem
                      ...
                      link={mainNavigationItem?.link}
                      label={mainNavigationItem?.label}
                      cssClass={`flex items-center font-bold text-text-overlay opacity-90 hover:text-text-color hover:opacity-100 border-1 border-primary cursor-pointer bg-primary px-2 xl:px-4 rounded transition-all uppercase ${
                        mainNavigationItem?.cssClass
                      } ${
                        small
                          ? "text-xs xl:text-sm py-[0.15rem] xl:py-1"
                          : "text-xs lg:text-sm xl:text-base py-1 xl:py-2"
                      }`}
                      sameTab={mainNavigationItem?.sameTab}
                    />
                  ))}
              ...
              <SocialMediaIcons
                fadeDirection="down"
                siteLibrary={siteLibrary}
                cssClass="max-w-max xl:flex flex-row social-icons-row items-center justify-center text-primary gap-x-2 hidden"
              />
