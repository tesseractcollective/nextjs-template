      ...
      <div className="flex flex-row flex-wrap w-full mx-0 transition">
        {gridBoxDataWLinks.map((gridBoxItem, index) => {
          ...
          return (
            <LinkItem
              key={index}
              parentCssClass={`relative w-full lg:w-[50%] h-100vh group overflow-hidden transition-all ${
                isHovered ? "grayscale" : ""
              }`}
        ...
        {gridBoxDataWOLinks.map((gridBoxItem, index) => {
          ...
          return (
            <div
              key={index}
              className={`relative w-full lg:w-[50%] h-100vh group overflow-hidden transition-all ${
                isHovered ? "grayscale" : ""
              }`}
              onMouseEnter={() => setHoveredItemIndex(index)}
              onMouseLeave={() => setHoveredItemIndex(null)}
