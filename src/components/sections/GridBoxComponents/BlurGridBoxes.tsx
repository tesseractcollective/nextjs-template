...
export default function BlurGridBoxes({ gridBoxData }: GridBoxProps) {
    ...
    const filterFilloutCards = gridBoxData.filter((item) => item.boxLink?.includes("fillout:"));
    const filterLinkCards = gridBoxData.filter((item) => !item.boxLink?.includes("fillout:"));
  return (
    <Fade triggerOnce className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
      <div className={`gap-8 grid grid-cols-1 md:grid-cols-2 ${arrayQuantityUnderTwo ? 'lg:grid-cols-2':'lg:grid-cols-3'}`}>
          ...
          >
            ...
            <div className="p-4 h-full">
              <div className="flex flex-row justify-between items-center w-full mb-1">
                ...
                {gridBoxItem?.boxLink && (
                  <span
                    className="inline-block text-white font-semibold transition-all duration-300 ease-in-out group-hover:text-primary px-2 group-hover:pl-4 group-hover:pr-0 group-hover:scale-x-110"
                  >
                    <ArrowLongRightIcon
                      className="h-8 w-8 flex"
                      aria-hidden="true"
