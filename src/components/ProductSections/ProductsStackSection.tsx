...
export default function ProductsStackSection({ products }: ProductsProps) {
              ...
              >
                <div className="flex flex-col w-full">
                  <div className="flex flex-row items-start justify-between gap-x-6">
                    {product.name && (
                      <div className="text-sm lg:text-md font-bold my-0 py-0 parsed-mb-0 uppercase tracking-widest text-left text-text-color">
                        {parse(product.name)}
                      </div>
                    )}
                    <div className="flex-grow border-b border-dotted border-text-color/30 mx-2 h-[1px] self-center"></div>
                    {product.price && (
                ...
                <FontAwesomeIcon
                  ...
                  className="fa-fw my-0 py-0  h-5 w-5 absolute right-[-10px] bottom-[-10px] opacity-0 group-hover:opacity-100 transition-all bg-primary rounded-full"
                />
              </button>
            ))}
          </div>
        ...
        {open && (
          <div onClick={() => setOpen(false)}>
            <motion.div
              ...
              {...{
                className:
                  "bg-[#00000070] backdrop-blur-sm fixed inset-0 z-[1000] grid place-items-center overflow-y-scroll cursor-pointer p-6",
              }}
            >
              <div
                className="relative w-full mx-auto flex max-w-xl"
              ...
              >
                <motion.div
                  ...
                  {...{
                    className:
                      "bg-text-color text-dark py-4 px-2 rounded-lg w-full max-w-xl shadow-xl cursor-default relative overflow-hidden dialog-popup-page max-h-[80dvh]",
                  }}
                >
